...
def load_dataset(config: DataConfig) -> FxDataset:
    ...
    if full_df is None or full_df.empty:
        raise ValueError("Dataset dataframe is empty; cannot run regression plugin.")
...
def resolve_columns(
    dataset: FxDataset, requested_targets: list[str]
) -> tuple[list[str], list[str]]:
    available = set(dataset.full_df.columns)
    targets = [c for c in requested_targets if c in available]
    missing_targets = [c for c in requested_targets if c not in available]
    if missing_targets:
        raise ValueError(f"Target columns missing from dataset: {missing_targets}")
