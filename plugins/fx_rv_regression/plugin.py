...
class FxRvRegressionPlugin(Plugin, ConfigurablePlugin):
    @property
    ...
    def run(self, spec: RunSpec, *, context: RunContext) -> RunResult:
        ...
        _validate_dataset_columns(dataset.full_df.columns, config)
        splits = split_by_date(dataset.full_df, index_col=dataset.index_col, split_cfg=config.split)

        horizon_results = run_experiment(config, splits, seed=spec.seed)
        ...
        summary_path = write_data_summary(
            ...
            experiment_name=canonical_experiment_name,
            target_cols=config.experiment.target_cols,
        )
...
def _ensure_dependencies() -> None:
    ...
    missing: list[str] = []
    for module in ("mlh_data", "pandas", "numpy", "pyarrow", "sklearn", "joblib"):
        if importlib.util.find_spec(module) is None:
            missing.append(module)
    if missing:
        raise RuntimeError(
            "Missing dependencies for FxRvRegressionPlugin: "
            f"{', '.join(missing)}. "
            'Install with `pip install -e ".[mlh-data,data,sklearn]"`.'
        )


def _validate_dataset_columns(columns: Sequence[str], config: FxRvRegressionConfig) -> None:
    column_set = set(columns)
    required = required_columns_for_experiment(
        config.experiment.name, config.experiment.target_cols
    )
